...
async function queryStravaByField(field, startDate, endDate) {
  try {
    const snap = await adminDb.collection("stravaActivities")
...
function normaliseStravaActivity(activity) {
  const dateValue = activity.date || activity.startDate || activity.start_date_local || activity.start_date
  const date = typeof dateValue === "string" ? dateValue.slice(0, 10) : null
  const seconds = activity.moving_time || activity.elapsed_time || activity.movingTime || activity.elapsedTime || 0
  const distanceMeters = activity.distance || activity.distanceMeters || 0
  return {
    id: activity.id,
    date,
    name: activity.name || activity.title || "",
    type: activity.type || activity.sport_type || activity.sportType || "",
    distanceKm: distanceMeters ? Math.round((Number(distanceMeters) / 1000) * 10) / 10 : null,
    movingMinutes: seconds ? Math.round(Number(seconds) / 60) : null,
  }
}

async function getStravaSummary(startDate, endDate) {
  const [byDate, byStartDate, byStartLocal] = await Promise.all([
    queryStravaByField("date", startDate, endDate),
    queryStravaByField("startDate", startDate, endDate),
    queryStravaByField("start_date_local", `${startDate}T00:00:00`, `${endDate}T23:59:59`),
  ])

  const seen = new Set()
  const activities = [...byDate, ...byStartDate, ...byStartLocal]
    .filter((activity) => {
      const key = String(activity.id || activity.externalId || JSON.stringify(activity))
  ...
  return {
    ...
    activities,
    totals: {
      movingMinutes: activities.reduce((sum, a) => sum + (a.movingMinutes || 0), 0),
      distanceKm: Math.round(activities.reduce((sum, a) => sum + (a.distanceKm || 0), 0) * 10) / 10,
    },
    note: activities.length > 0
      ? "Matched Strava activities from Firestore collection stravaActivities."
      : "No Strava activities found yet. When imported into stravaActivities, this review will include them.",
  }
}
...
export async function getTrainingRealitySnapshot(options = {}) {
  ...
  const strava = await getStravaSummary(rangeStart, rangeEnd)
  ...
  return {
    ...
    currentShape: {
      ...
      strava,
      note: "Shape is inferred from plan adherence, logged habits, sleep/weight, and optional Strava imports. Checked sessions confirm completion, but not pace, HR, power, or RPE.",
    },
    dayLoad: {
      label: getLoadLabel({
