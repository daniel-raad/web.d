...
GOALS — source of truth: Call get_goals at the start of any goal-relevant conversation. The goals collection holds his active goals; never hardcode targets, deadlines, or thresholds. Each goal has a TYPE that tells you how to evaluate progress:
...
export const READ_TOOLS = [
  ...
  },
  {
    ...
    description: "The single most important tool. Returns Daniel's whole picture in one call: current month revenue vs £10k, top Revenue todos, last-7-days training load, recent sleep average, days left in the month, AND his Ironman block — days to race, this week's progress per discipline (swim/bike/run/strength) vs targets. Call this at the start of every check-in so suggestions are anchored to what actually moves him toward £10k while training for Ironman 70.3. Costs one round trip — use it freely.",
    input_schema: { type: "object", properties: {} },
  },
  {
  ...
  {
    name: "get_goals",
    description: "Get Daniel's structured goals — the spine of the planner. Each goal has a `type` (deadline-plan, outcome-leads, process-cadence) that tells you how to evaluate progress. The goals collection is the source of truth — NEVER hardcode targets, deadlines, or thresholds; read them from here. Returns id, type, title, status, plus type-specific fields (deadline + weeklyTargets for deadline-plan; target + current + leadMeasures for outcome-leads; floor + target + primaryPrimitive for process-cadence).",
    input_schema: {
      type: "object",
      properties: {
        status: { type: "string", description: "Filter by status. Defaults to 'active'. Pass 'all' to include everything." },
      },
    },
  },
  {
...
export async function executeTool(name, input) {
  switch (name) {
    ...
    case "get_goals": {
      ...
      const filtered = wantStatus === "all" ? goals : goals.filter((g) => g.status === wantStatus)
      filtered.sort((a, b) => (a.priority || 99) - (b.priority || 99))
      return { count: filtered.length, goals: filtered }
    }
