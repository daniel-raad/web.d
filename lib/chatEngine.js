...
import {
  searchNotes as vaultSearchNotes,
  readNote as vaultReadNote,
  writeNote as vaultWriteNote,
  appendNote as vaultAppendNote,
  findBacklinks as vaultFindBacklinks,
} from "./obsidianVault"
...
- MATCH HIS ENERGY. When Daniel comes in with intent ("locked in", "smashing today", "let's go"), reinforce the move — name the FIRST concrete action and get out of the way. Do NOT re-list the gap, do NOT remind him what's behind, do NOT cool the moment with caveats. The data already said what's short; he's responding. Keep momentum, don't relitigate. Push back HARDER only if the data shows he's genuinely overcooked (sleep collapsing + HR drift + energy ≤2 for 3+ days) — one line, then back to the plan.
...
VAULT — Daniel's Obsidian vault lives in a GitHub repo (he opens the same repo locally in Obsidian). Capture his thoughts as notes here, especially from voice notes and photos. Tools: vault_search (snippets only — call before reading), vault_read (full content — only when you need it), vault_append (preferred for adding to an existing note), vault_write (create new), vault_backlinks (find notes that reference a title). Use [[wikilinks]] freely — Obsidian derives backlinks from them at view time, you do NOT need to track backlinks yourself. Folder convention: daily/YYYY-MM-DD.md for daily journaling, people/Name.md, topics/topic-slug.md, notes/ for general. Default behavior: a stream-of-thought voice note → append to today's daily note; a clear topic ("idea for Conversify pricing") → append or create the topic note and link people/projects with [[wikilinks]]. Keep notes terse; Obsidian rewards short, well-linked entries over long monoliths.
...
// Obsidian-style vault tools. Backed by a private GitHub repo Daniel opens in
// Obsidian locally. Kept as a separate array so callers can opt in (chat.js
// only enables them for the authed user, telegram.js enables them always).
export const VAULT_TOOLS = [
  {
    name: "vault_search",
    description:
      "Search Daniel's Obsidian vault by title or content. Returns only path + title + a short snippet (≤160 chars) for each hit — TOKEN-FRUGAL on purpose. Call this BEFORE vault_read so you only fetch full notes you actually need. Supports GitHub code-search qualifiers (e.g. 'path:topics/' or 'in:title pricing').",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "vault_read",
    description:
      "Read a single vault note by path. Returns frontmatter + full body. Use sparingly — prefer vault_search snippets when you can answer from those. Path is relative to the vault root, e.g. 'daily/2026-06-17.md' or 'topics/conversify-pricing.md'. The '.md' extension is added if missing.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "vault_append",
    description:
      "Append a block to an existing note. PREFERRED over vault_write for adding to a note that already exists — never reads or rewrites the full file. Use a section heading when the addition is its own topic (creates a '## Section' header), otherwise leave it off for a free-floating block.",
    input_schema: {
      type: "object",
      properties: {
        path: { type: "string", description: "Existing note path." },
        section: { type: "string", description: "Optional heading inserted as '## section'." },
        body: { type: "string", description: "Markdown body to append. Use [[wikilinks]] freely." },
      },
      required: ["path", "body"],
    },
  },
  {
    name: "vault_write",
    description:
      "Create a new note. If 'path' is omitted, derives one from the title under 'notes/'. Fails if the note already exists unless overwrite=true. Use for genuinely new notes; for adding to existing ones use vault_append.",
    input_schema: {
      type: "object",
      properties: {
        path: { type: "string", description: "Optional explicit path, e.g. 'topics/pricing.md'." },
        title: { type: "string", description: "Note title (used in frontmatter and to derive path if omitted)." },
        body: { type: "string", description: "Markdown body. Use [[wikilinks]] to connect to other notes." },
        tags: { type: "array", items: { type: "string" }, description: "Optional Obsidian tags." },
        aliases: { type: "array", items: { type: "string" }, description: "Optional aliases (alternate titles)." },
        overwrite: { type: "boolean", description: "If true, replaces an existing note. Default false." },
      },
      required: ["body"],
    },
  },
  {
    ...
    description:
      "Find notes that link to a given title via [[wikilink]]. Returns the same compact snippet shape as vault_search. Use when Daniel asks 'what mentions X?' or before renaming a note to see what would break.",
    input_schema: {
      type: "object",
      properties: {
        title: { type: "string", description: "The note title to search backlinks for (without the [[ ]] brackets)." },
...
export async function executeTool(name, input) {
  switch (name) {
    case "vault_search": {
      try {
        return await vaultSearchNotes(input.query, { limit: input.limit })
      } catch (e) {
        return { error: `Vault search failed: ${e.message}` }
      }
    }
    case "vault_read": {
      try {
        return await vaultReadNote(input.path)
      } catch (e) {
        return { error: `Vault read failed: ${e.message}` }
      }
    }
    case "vault_append": {
      try {
        return await vaultAppendNote({
          path: input.path,
          section: input.section,
          body: input.body,
        })
      } catch (e) {
        return { error: `Vault append failed: ${e.message}` }
      }
    }
    case "vault_write": {
      try {
        return await vaultWriteNote({
          path: input.path,
          title: input.title,
          body: input.body,
          tags: input.tags,
        ...
        return { error: `Vault write failed: ${e.message}` }
      }
    }
    case "vault_backlinks": {
      try {
        return await vaultFindBacklinks(input.title, { limit: input.limit })
      } catch (e) {
        return { error: `Vault backlinks failed: ${e.message}` }
      }
    }
    case "get_today": {
