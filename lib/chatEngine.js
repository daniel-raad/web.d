...
import { WEEKS, PHASES, DISCIPLINE_INFO, getCurrentWeek, getWeekWithDates } from "../components/Todos/ironmanData"
...
export const READ_TOOLS = [
  ...
  },
  {
    ...
    description: "Get Daniel's Ironman 70.3 training plan including workout details, checked-off sessions, and progress. Can fetch a specific week, today's sessions only, or a summary of all weeks.",
    input_schema: {
      type: "object",
      properties: {
...
export async function executeTool(name, input) {
  switch (name) {
    case "get_today": {
      ...
      const dueTodos = allActive
        ...
        .map((t) => ({
          ...
          category: t.category,
          dueDate: t.dueDate,
          overdueDays: Math.floor((new Date(today + "T00:00:00") - new Date(t.dueDate + "T00:00:00")) / (1000 * 60 * 60 * 24)),
        }))
      ...
      const planData = ironmanDoc.exists ? ironmanDoc.data() : { checked: {}, startDate: "2026-03-30", dayOrders: {}, sessionMoves: {} }
      const { checked = {}, startDate = "2026-03-30", dayOrders = {}, sessionMoves = {} } = planData
      ...
      if (weekData) {
        ...
        const diffDays = Math.round((now - weekStart) / (1000 * 60 * 60 * 24))

        if (diffDays >= 0 && diffDays < weekData.days.length) {
          const order = dayOrders[currentWeek] || Array.from({ length: weekData.days.length }, (_, i) => i)
          const origDayIdx = diffDays < order.length ? order[diffDays] : diffDays

          weekData.days.forEach((day, di) => {
            day.sessions.forEach((session, si) => {
              const key = `w${currentWeek}-d${di}-s${si}`
    ...
    case "get_ironman_plan": {
      ...
      const planData = doc.exists ? doc.data() : { checked: {}, startDate: "2026-03-30", dayOrders: {}, sessionMoves: {} }
      const { checked = {}, startDate, dayOrders = {}, sessionMoves = {} } = planData
      const currentWeek = getCurrentWeek(startDate)
      const today = new Date().toISOString().split("T")[0]

      if (input.today) {
        // Return only today's sessions, respecting custom day/session reordering
        const weekData = WEEKS.find((w) => w.week === currentWeek)
        ...
        if (diffDays < 0 || diffDays >= weekData.days.length) {
          return { today, currentWeek, sessions: [], note: "Today falls outside the current training week" }
        }
    ...
    case "get_completed_todos": {
      ...
      const snap = await adminDb.collection("todos")
        ...
        .get()
      return snap.docs.map((d) => ({ id: d.id, ...d.data() })).sort((a, b) => (b.completedAt || 0) - (a.completedAt || 0))
    }
    ...
    case "toggle_ironman_session": {
      ...
      const data = doc.exists ? doc.data() : { checked: {}, startDate: "2026-03-30", dayOrders: {}, sessionMoves: {} }
      const checked = { ...(data.checked || {}), [key]: value }
      await ref.set({ ...data, checked })
      return { ok: true, key, value }
    }
    case "move_ironman_session": {
      const { key, targetDayIdx } = input
      if (!key || !/^w\d+-d\d+-s\d+$/.test(key)) return { error: "Invalid session key — expected format 'w{week}-d{day}-s{session}'" }
      ...
      if (targetDayIdx === origDayIdx) {
        delete sessionMoves[key]
      } else {
        sessionMoves[key] = targetDayIdx
      }
      await ref.set({ ...data, sessionMoves })
      return { ok: true, key, targetDayIdx, cleared: targetDayIdx === origDayIdx }
    }
    ...
    case "reset_ironman_week_order": {
      ...
      for (const [k, v] of Object.entries(data.sessionMoves || {})) {
        if (!k.startsWith(`w${week}-`)) sessionMoves[k] = v
      }
