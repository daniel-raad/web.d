...
export const WRITE_TOOLS = [
  ...
  },
  {
    ...
    description: "Move a single training session to a different day, even across weeks (e.g. shift today's run to tomorrow, or push a missed swim from week 3 into week 4). The targetDayIdx is the original day index 0-6 within the target week's days array. Optionally pass targetWeek to move across weeks. Pass the original day and week to clear a previous move.",
    input_schema: {
      type: "object",
      properties: {
        key: { type: "string", description: "Session key, e.g. 'w3-d2-s0'" },
        targetDayIdx: { type: "number", description: "Original day index within the target week (0-6)" },
        targetWeek: { type: "number", description: "Target week number (1-21). Defaults to the session's original week if omitted." },
      },
