...
import { WEEKS, PHASES } from "../components/Todos/ironmanData"
...
export async function executeTool(name, input) {
  switch (name) {
    ...
    case "get_ironman_plan": {
      ...
      const now = new Date()
      const currentWeek = Math.max(1, Math.min(24, Math.ceil((now - start) / (7 * 24 * 60 * 60 * 1000)) + 1))

      if (input.week) {
        // Return detailed workout data for a specific week
        const weekData = WEEKS.find(w => w.week === input.week)
        if (!weekData) return { error: `Week ${input.week} not found (valid: 1-24)` }

        // Add checked status to each session
        const days = weekData.days.map((day, di) => ({
          ...day,
          sessions: day.sessions.map((session, si) => ({
            ...session,
            checked: !!checked[`w${input.week}-d${di}-s${si}`],
          })),
        }))
      ...
      const summary = WEEKS.map(w => {
        const totalSessions = w.days.reduce((sum, d) => sum + d.sessions.length, 0)
        const completedSessions = w.days.reduce((sum, d, di) =>
          sum + d.sessions.filter((_, si) => checked[`w${w.week}-d${di}-s${si}`]).length, 0)
        return {
          week: w.week,
          phase: w.phase,
          title: w.title,
