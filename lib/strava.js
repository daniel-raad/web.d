...
import { getDateKey, compareDateKeys, getDayNameForDateKey } from "./dates.js"
...
function summarizeActivity(a, todayKey = getDateKey()) {
  // start_date_local is wall-clock at the activity location (no TZ suffix).
  // Slice the date portion so we get a stable local-day key regardless of how
  // Date parses it on the server.
  const date = typeof a.start_date_local === "string" ? a.start_date_local.slice(0, 10) : null
  const daysAgo = date ? compareDateKeys(todayKey, date) : null
  const dayLabel = (() => {
  ...
  const weekday = date ? getDayNameForDateKey(date) : null
  return {
    id: a.id,
    name: a.name,
...
export async function getRecentActivities({ days = 14, perPage = 30 } = {}) {
  ...
  return data.map((a) => summarizeActivity(a, todayKey))
}
