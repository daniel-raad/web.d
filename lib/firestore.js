...
export async function getIronmanPlan() {
  const res = await fetch("/api/ironman")
  return res.json()
}

export async function getIronmanReview(rangeDays = 14) {
  const res = await fetch(`/api/ironman/review?rangeDays=${rangeDays}`)
  return res.json()
}

export async function saveIronmanPlan(data) {
  await fetch("/api/ironman", {
    method: "POST",
    headers: await authHeaders(),
    body: JSON.stringify(data),
  })
...
export async function resetIronmanPlan(data) {
  await fetch("/api/ironman", {
    method: "PUT",
    headers: await authHeaders(),
    body: JSON.stringify(data),
